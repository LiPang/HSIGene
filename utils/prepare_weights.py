...
def integrate(local_weights, global_weights, config_path, output_path):
    ...
    for sk in scratch_dict.keys():
        ...
        if sk in local_weights and sk in global_weights:
            ...
            if (local_weights[sk].equal(global_weights[sk])):
                target_dict[sk] = local_weights[sk].clone()
            else:
                print("sk:",sk," is not equal")
                # print(local_weights[sk])
                target_dict[sk] = global_weights[sk].clone()
        elif 'local_adapter' in sk:
            # print(sk)
            if (sk not in local_weights.keys()):
                print("missing:",sk)
            target_dict[sk] = local_weights[sk].clone()
        elif 'global_adapter' or "metadata" in sk:
            if (sk not in global_weights.keys()):
                print("missing:",sk)
            target_dict[sk] = global_weights[sk].clone()
        else:
            raise ValueError()
